...
try:
    from eval.metric import ImageEvaluator, MetricConfig
    METRIC_EVALUATOR_AVAILABLE = True
except ImportError:
    METRIC_EVALUATOR_AVAILABLE = False
    print("Warning: metric evaluator not available, will use MSE-based rewards")
...
def add_virtual_human_scores(batch):
    """
    为原始数据添加虚拟人工分数，用于GRPO训练
    ...
    """
    # 为每个样本添加虚拟的人工评分
    batch_size = len(batch["ref"])
    
    # 生成虚拟的人工评分 (0-10分)
    human_scores = {
        "removal_quality": torch.rand(batch_size) * 10,  # 移除质量分数
        "background_naturalness": torch.rand(batch_size) * 10,  # 背景自然度分数
        "overall_quality": torch.rand(batch_size) * 10,  # 整体质量分数
    }
    
    batch["human_scores"] = human_scores
    return batch


def compute_grpo_rewards(pred_latents, target_latents, mask_latents, args, vae=None, pipeline=None, debug_step=None):
    """
    计算GRPO奖励，基于传统图像质量指标（SSIM、PSNR、LPIPS）
    
    注意：在Flux训练中，pred_latents和target_latents是噪声预测，不是直接的图像 latent。
    要计算真实的图像质量指标，需要完整的pipeline去噪和后处理。
    如果没有pipeline，则回退到基于噪声预测的MSE奖励。
    
    Args:
        pred_latents: 预测的噪声 [batch_size, ...]
        target_latents: 目标噪声 [batch_size, ...]
        mask_latents: 掩码latent表示 [可选]
        args: 训练参数
        vae: VAE解码器
        pipeline: 完整的推理pipeline（用于将噪声预测转换为图像）
    ...
    # 如果没有metric evaluator或无法进行图像质量评估，回退到基础MSE奖励
    if not METRIC_EVALUATOR_AVAILABLE or (vae is None and pipeline is None):
        logger.warning("无法进行图像质量评估，使用MSE奖励")
        return _compute_mse_fallback_rewards(pred_latents, target_latents, args)
    
    # 初始化评估器（只在训练时使用轻量级指标）
    if not hasattr(compute_grpo_rewards, 'evaluator'):
        config = MetricConfig(
            enable_psnr=True,     # 快速，基于像素差异
            enable_ssim=True,     # 中等速度，结构相似性
            enable_lpips=True,    # 感知质量（如果可用）
            enable_fid=False,     # 太慢，训练时不使用
            enable_clip=False,    # 太慢，训练时不使用
            enable_dino=False,    # 太慢，训练时不使用
            device=device,
            batch_size=1
        )
        compute_grpo_rewards.evaluator = ImageEvaluator(config)
    
    evaluator = compute_grpo_rewards.evaluator
    
    # 尝试将噪声预测转换为图像进行质量评估
    with torch.no_grad():
        try:
            if pipeline is not None:
                # 使用完整pipeline进行推理（理想情况）
                logger.info("使用完整pipeline进行图像质量评估")
                # 这里需要实现完整的pipeline推理逻辑
                # 由于复杂性，现在先回退到VAE解码
                raise NotImplementedError("完整pipeline推理尚未实现")
                
            elif vae is not None:
                # 控制调试输出频率：只在前10步、每100步或特定debug_step时打印详细信息
                should_debug = (debug_step is not None and 
                               (debug_step < 10 or debug_step % 100 == 0))
                
                if should_debug:
                    logger.info("使用Flux标准解码流程：unpack + VAE decode")
                
                # 检测VAE数据类型
                vae_dtype = next(vae.parameters()).dtype
                if should_debug:
                    logger.info(f"VAE数据类型: {vae_dtype}, 输入latents类型: {pred_latents.dtype}")
                
                # 类型转换
                if pred_latents.dtype != vae_dtype:
                    pred_latents_vae = pred_latents.to(vae_dtype)
                    target_latents_vae = target_latents.to(vae_dtype)
                else:
                    pred_latents_vae = pred_latents
                    target_latents_vae = target_latents
                
                # 推断图像尺寸（从 latents 形状推断）
                batch_size, num_patches, channels = pred_latents_vae.shape
                if should_debug:
                    logger.info(f"Latents形状: [{batch_size}, {num_patches}, {channels}]")
                
                # 根据num_patches推断图像尺寸
                if num_patches == 2048:  # 512x1024 或 1024x512
                    # 根据约定，假设是 512x1024 (h x 2*w)
                    height, width = 512, 1024
                elif num_patches == 1024:  # 512x512
                    height, width = 512, 512
                elif num_patches == 512:   # 256x512
                    height, width = 256, 512
                else:
                    # 默认尺寸
                    height, width = 512, 512
                    if should_debug:
                        logger.warning(f"未知的num_patches: {num_patches}，使用默认尺寸 {height}x{width}")
                
                if should_debug:
                    logger.info(f"推断图像尺寸: {height}x{width}")
                
                # 使用Flux标准的_unpack_latents函数
                vae_scale_factor = 8  # Flux标准的VAE scale factor
                pred_latents_unpacked = _unpack_latents(pred_latents_vae, height, width, vae_scale_factor)
                target_latents_unpacked = _unpack_latents(target_latents_vae, height, width, vae_scale_factor)
                
                if should_debug:
                    logger.info(f"Unpacked latents形状: {pred_latents_unpacked.shape}")
                
                # 按照FluxFillPipeline的标准流程进行VAE解码
                # 1. 应用scaling和shift因子
                scaling_factor = getattr(vae.config, 'scaling_factor', 0.3611) if hasattr(vae, 'config') else 0.3611
                shift_factor = getattr(vae.config, 'shift_factor', 0.0) if hasattr(vae, 'config') else 0.0
                
                if should_debug:
                    logger.info(f"VAE配置: scaling_factor={scaling_factor}, shift_factor={shift_factor}")
                
                pred_latents_scaled = (pred_latents_unpacked / scaling_factor) + shift_factor
                target_latents_scaled = (target_latents_unpacked / scaling_factor) + shift_factor
                
                # 2. VAE解码
                pred_images = vae.decode(pred_latents_scaled, return_dict=False)[0]
                target_images = vae.decode(target_latents_scaled, return_dict=False)[0]
                
                if should_debug:
                    logger.info(f"解码后图像形状: {pred_images.shape}")
            ...
            # 如果解码失败，回退到MSE奖励
            logger.warning(f"图像解码失败，使用MSE奖励: {e}")
            return _compute_mse_fallback_rewards(pred_latents, target_latents, args)
    ...
    for i in range(batch_size):
        try:
            # 转换为PIL图像
            pred_pil = transforms.ToPILImage()(pred_images[i].cpu())
            target_pil = transforms.ToPILImage()(target_images[i].cpu())
            
            # 计算PSNR
            psnr = evaluator.calculate_psnr(
                np.array(pred_pil), np.array(target_pil)
            )
            psnr_scores.append(psnr if psnr is not None and not np.isnan(psnr) else 20.0)
            
            # 计算SSIM
            ssim = evaluator.calculate_ssim(
                np.array(pred_pil), np.array(target_pil)
            )
            ssim_scores.append(ssim if ssim is not None and not np.isnan(ssim) else 0.5)
            
            # 计算LPIPS（如果可用）
            if evaluator.config.enable_lpips:
                lpips_val = evaluator.calculate_lpips(pred_pil, target_pil)
                lpips_scores.append(lpips_val if lpips_val is not None and not np.isnan(lpips_val) else 0.5)
            else:
                lpips_scores.append(0.5)  # 默认值
                
        except Exception as e:
            logger.warning(f"计算第{i}个样本的质量指标时出错: {e}")
            # 使用默认值
            psnr_scores.append(20.0)
            ssim_scores.append(0.5)
            lpips_scores.append(0.5)
    
    # 将指标转换为奖励分数（0-10分）
    def psnr_to_score(psnr):
        # PSNR: 10-50dB -> 0-10分
        return max(0.0, min(10.0, (psnr - 10) / 4.0))
    
    def ssim_to_score(ssim):
        # SSIM: 0-1 -> 0-10分
        return max(0.0, min(10.0, ssim * 10.0))
    
    def lpips_to_score(lpips_val):
        # LPIPS: 0-1 (越小越好) -> 0-10分
        return max(0.0, min(10.0, (1.0 - lpips_val) * 10.0))
    
    # 计算各项奖励分数
    psnr_rewards = torch.tensor([psnr_to_score(p) for p in psnr_scores], device=device)
    ssim_rewards = torch.tensor([ssim_to_score(s) for s in ssim_scores], device=device)
    lpips_rewards = torch.tensor([lpips_to_score(l) for l in lpips_scores], device=device)
    
    # 组合奖励（归一化到0-1范围）
    psnr_rewards = psnr_rewards / 10.0
    ssim_rewards = ssim_rewards / 10.0
    lpips_rewards = lpips_rewards / 10.0
    
    # 基于不同指标计算三种奖励
    removal_reward = (
        0.5 * psnr_rewards +   # PSNR关注像素级精度
        0.3 * ssim_rewards +   # SSIM关注结构
        0.2 * lpips_rewards    # LPIPS关注感知质量
    ) * args.removal_reward_weight
    
    background_reward = (
        0.3 * psnr_rewards +
        0.4 * ssim_rewards +   # 背景更关注结构一致性
        0.3 * lpips_rewards
    ) * args.background_reward_weight
    
    smoothness_reward = (
        0.2 * psnr_rewards +
        0.5 * ssim_rewards +   # 平滑度主要看结构
        0.3 * lpips_rewards
    ) * args.smoothness_reward_weight
    
    rewards = {
        "removal_reward": removal_reward,
        "background_reward": background_reward,
        "smoothness_reward": smoothness_reward,
    }
    
    # 计算总奖励
    total_reward = removal_reward + background_reward + smoothness_reward
...
def _unpack_latents(latents, height, width, vae_scale_factor):
    """
    Unpack latents from Flux format to standard VAE format.
...
def _compute_mse_fallback_rewards(pred_latents, target_latents, args):
    """
    回退到基于噪声预测的MSE奖励计算
    """
    batch_size = pred_latents.shape[0]
    ...
    # 自适应维度处理
    if len(mse_loss.shape) == 4:  # [B, C, H, W]
        mse_per_sample = mse_loss.mean(dim=[1, 2, 3])
    elif len(mse_loss.shape) == 3:  # [B, L, D] 
        mse_per_sample = mse_loss.mean(dim=[1, 2])
    elif len(mse_loss.shape) == 2:  # [B, D]
        mse_per_sample = mse_loss.mean(dim=1)
    else:
        mse_per_sample = mse_loss.view(batch_size, -1).mean(dim=1)
    
    # 将MSE转换为奖励
    base_reward = 1.0 / (1.0 + mse_per_sample)
    
    rewards = {
        "removal_reward": base_reward * args.removal_reward_weight,
        "background_reward": base_reward * args.background_reward_weight,
        "smoothness_reward": base_reward * args.smoothness_reward_weight,
    }
...
def compute_grpo_loss(pred, target, old_pred, rewards, args):
    """
    计算GRPO损失函数
    
    Args:
        pred: 当前策略的预测，形状 [batch_size, seq_len, hidden_dim]
        target: 目标latent，形状 [batch_size, seq_len, hidden_dim]
        old_pred: 旧策略的预测，形状 [batch_size, seq_len, hidden_dim]
        rewards: 奖励字典
        args: 训练参数
    ...
    # 计算策略比率（简化版本，基于MSE差异）
    current_mse = F.mse_loss(pred, target, reduction='none').mean(dim=[1, 2])
    old_mse = F.mse_loss(old_pred, target, reduction='none').mean(dim=[1, 2])
    
    # 策略比率（MSE越小越好，所以用倒数）
    ratio = (1.0 + old_mse) / (1.0 + current_mse + 1e-8)
    ...
    # 策略损失
    policy_loss1 = -total_reward * ratio
    policy_loss2 = -total_reward * clipped_ratio
    policy_loss = torch.max(policy_loss1, policy_loss2).mean()
    
    # 多样性损失（鼓励不同样本之间的差异）
    pred_flat = pred.view(pred.shape[0], -1)
    if pred_flat.shape[0] > 1:
        # 只有当batch_size > 1时才计算多样性损失
        diversity_loss = -torch.pdist(pred_flat).mean()
    else:
        # batch_size = 1时，多样性损失设为0
        diversity_loss = torch.tensor(0.0, device=pred.device, dtype=pred.dtype)
...
def parse_args():
    ...
    parser.add_argument("--use_grpo", action="store_true", default=False,
                        help="启用GRPO训练模式")
    parser.add_argument("--enable_grpo_loss", action="store_true", default=False,
                        help="启用GRPO损失函数")
    parser.add_argument("--base_loss_weight", type=float, default=1.0,
                        help="基础MSE损失权重")
    parser.add_argument("--policy_loss_weight", type=float, default=0.1,
                        help="策略损失权重")
    parser.add_argument("--diversity_loss_weight", type=float, default=0.05,
                        help="多样性损失权重")
    parser.add_argument("--clip_range", type=float, default=0.2,
                        help="PPO裁剪范围")
    parser.add_argument("--removal_reward_weight", type=float, default=0.4,
                        help="移除奖励权重")
    parser.add_argument("--background_reward_weight", type=float, default=0.3,
                        help="背景奖励权重")
    parser.add_argument("--smoothness_reward_weight", type=float, default=0.3,
                        help="平滑奖励权重")
    ...
    if args.optimizer_config is None:
        ...
        elif args.optimizer_type.lower() == "adamw8bit" or args.use_8bit_optimizer:
            args.optimizer_config = {
                "type": "AdamW8bit",
                "params": {
                    "lr": args.learning_rate,
...
def main():
    ...
    if optimizer_type.lower() == "prodigy":
        ...
        try:
            ...
            optimizer = bnb.optim.AdamW8bit(
                list(trainable_params),
                lr=optimizer_params.get("lr", args.learning_rate),
                betas=optimizer_params.get("betas", (args.adam_beta1, args.adam_beta2)),
                weight_decay=optimizer_params.get("weight_decay", args.adam_weight_decay),
                eps=optimizer_params.get("eps", args.adam_epsilon)
            )
            logger.info(f"使用8-bit AdamW优化器，参数: {optimizer_params}")
        except ImportError:
            logger.warning("无法导入bitsandbytes，回退到标准AdamW")
            optimizer = torch.optim.AdamW(
                list(trainable_params),
                lr=args.learning_rate,
                betas=(args.adam_beta1, args.adam_beta2),
    ...
    # GRPO配置日志
    logger.info(f"  - GRPO训练模式: {'启用' if args.use_grpo else '禁用'}")
    if args.use_grpo:
        logger.info(f"  - GRPO损失函数: {'启用' if args.enable_grpo_loss else '禁用'}")
        if args.enable_grpo_loss:
            logger.info(f"    * 基础损失权重: {args.base_loss_weight}")
            logger.info(f"    * 策略损失权重: {args.policy_loss_weight}")
            logger.info(f"    * 多样性损失权重: {args.diversity_loss_weight}")
            logger.info(f"    * PPO裁剪范围: {args.clip_range}")
            logger.info(f"    * 移除奖励权重: {args.removal_reward_weight}")
            logger.info(f"    * 背景奖励权重: {args.background_reward_weight}")
            logger.info(f"    * 平滑奖励权重: {args.smoothness_reward_weight}")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(transformer):
                ...
                # GRPO训练模式
                if args.use_grpo and args.enable_grpo_loss:
                    # 添加虚拟人工分数（如果需要）
                    batch = add_virtual_human_scores(batch)
                    
                    # 计算奖励
                    rewards = compute_grpo_rewards(
                        pred_latents=pred,
                        target_latents=target,
                        mask_latents=mask_latents,  # 使用之前计算的mask_latents
                        args=args,
                        vae=vae,  # 传入VAE用于解码latent为图像
                        debug_step=global_step  # 传入步数以控制调试输出频率
                    )
                    
                    # 获取旧策略的预测（简化：使用当前预测的detached版本作为参考）
                    with torch.no_grad():
                        old_pred = pred.detach().clone()
                        # 添加一些噪声来模拟旧策略的差异
                        old_pred += torch.randn_like(old_pred) * 0.01
                    ...
                    if accelerator.is_main_process and global_step % 500 == 0:
                        logger.info(f"GRPO损失组件 - 总损失: {loss.item():.6f}, "
                                  f"基础损失: {loss_dict['base_loss'].item():.6f}, "
                                  f"策略损失: {loss_dict['policy_loss'].item():.6f}, "
                                  f"多样性损失: {loss_dict['diversity_loss'].item():.6f}")
                        logger.info(f"奖励 - 总奖励: {rewards['total_reward'].mean().item():.6f}, "
                                  f"移除奖励: {rewards['removal_reward'].mean().item():.6f}, "
                                  f"背景奖励: {rewards['background_reward'].mean().item():.6f}, "
                                  f"平滑奖励: {rewards['smoothness_reward'].mean().item():.6f}")
                else:
                    # 标准MSE损失
            ...
            if accelerator.is_main_process:
                ...
                # 如果使用GRPO，添加额外的日志信息
                if args.use_grpo and args.enable_grpo_loss:
                    logs["training_mode"] = "GRPO"
                else:
                    logs["training_mode"] = "Standard_MSE"
