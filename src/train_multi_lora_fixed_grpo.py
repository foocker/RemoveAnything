...
def compute_grpo_rewards(pred_latents, target_latents, mask_latents, args):
    """
    计算GRPO奖励（简化版本，基于latent空间的MSE）
    
    Args:
        pred_latents: 预测的latent，形状 [batch_size, seq_len, hidden_dim]
        target_latents: 目标latent，形状 [batch_size, seq_len, hidden_dim]
        mask_latents: mask的latent，形状 [batch_size, seq_len, hidden_dim]
        args: 训练参数
    ...
    # 基于latent空间的MSE计算简化奖励
    mse_loss = F.mse_loss(pred_latents, target_latents, reduction='none')
    mse_per_sample = mse_loss.mean(dim=[1, 2])  # [batch_size]
    
    # 将MSE转换为奖励（MSE越小，奖励越高）
