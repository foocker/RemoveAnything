...
from pipelines.flux.pipeline_flux_kontext_inpaint import FluxKontextInpaintPipeline
...
from data.triplet_bucket_dataset import TripletBucketDataset, triplet_collate_fn_simple
...
def main(args):
    ...
    # TODO: simple mode: simple_triplet
    train_dataset = TripletBucketDataset(args.train_json, args.buckets, mode=args.simple_triplet)
    batch_sampler = BucketBatchSampler(train_dataset, batch_size=args.train_batch_size, drop_last=False)
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset,
        batch_sampler=batch_sampler,
        collate_fn=triplet_collate_fn_simple,
    ...
    text_encoding_pipeline = FluxKontextInpaintPipeline.from_pretrained(
    ...
    if accelerator.is_main_process and args.report_to == "wandb" and args.log_dataset_samples:
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            control_images = batch["mask"]
            edited_image = batch["edited_image"]
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(flux_transformer):
                # Convert images to latent space following pipeline structure
                # Process input image (to be inpainted)
                input_image = batch["input_image"].to(accelerator.device, dtype=torch.float32)
                mask_image = batch["mask"].to(accelerator.device, dtype=torch.float32) 
                edited_image = batch["edited_image"].to(accelerator.device, dtype=torch.float32)
                
                # VAE encode images
                vae = vae.to(accelerator.device)
                
                # Encode input image (source image to be inpainted)
                input_latents = encode_images(input_image, vae, weight_dtype)
                
                # Encode target image (edited result)
                target_latents = encode_images(edited_image, vae, weight_dtype)
                
                # Process mask - following pipeline mask processing
                # Note: masked_image is created for reference but not used in current training logic
                # as we directly use input_latents and target_latents for the inpainting task
                
                # Process mask to latent space for loss computation with boundary handling
                ...
                # Check if soft mask is pre-computed in data or compute dynamically
                if "soft_mask" in batch:
                    # Use pre-computed soft mask from data processing
                    soft_mask_image = batch["soft_mask"].to(accelerator.device, dtype=torch.float32)
                    ...
                    # Compute soft mask dynamically (fallback or for experimentation)
                    import torch.nn.functional as F
                    kernel_size = 3
                    padding = kernel_size // 2
                    
                    # Dilate mask to create boundary region
                    dilate_kernel = torch.ones(1, 1, kernel_size, kernel_size, device=mask_latent.device, dtype=mask_latent.dtype)
                    expanded_mask = F.conv2d(mask_latent, dilate_kernel, padding=padding)
                    expanded_mask = torch.clamp(expanded_mask, 0, 1)
                    
                    # Create soft boundary mask using Gaussian blur for smoother transitions
                    sigma = 1.0
                    kernel_size_blur = 5
                    padding_blur = kernel_size_blur // 2
                    
                    # Create Gaussian kernel
                    x = torch.arange(kernel_size_blur, dtype=mask_latent.dtype, device=mask_latent.device)
                    x = x - kernel_size_blur // 2
                ...
                # Convert to same dtype as latents
                mask_latent = mask_latent.to(device=accelerator.device, dtype=weight_dtype)
                soft_mask = soft_mask.to(device=accelerator.device, dtype=weight_dtype)
                ...
                noise = torch.randn_like(target_latents, device=accelerator.device, dtype=weight_dtype)
                ...
                # Add noise according to flow matching to target latents
                ...
                # Following pipeline structure: concatenate noisy target with input image latents
                # This represents the inpainting task: predict clean target from noisy target + input context
                concatenated_noisy_model_input = torch.cat([noisy_target_latents, input_latents], dim=1)

                # pack the latents.
                packed_noisy_model_input = FluxKontextInpaintPipeline._pack_latents(
                ...
                latent_image_ids = FluxKontextInpaintPipeline._prepare_latent_image_ids(
                    bsz,
                ...
                # Unpack the model prediction to match target latents dimensions
                model_pred = FluxKontextInpaintPipeline._unpack_latents(
                    model_pred,
                ...
                # flow-matching loss for inpainting - IMPROVED MASK-AWARE LOSS WITH BOUNDARY HANDLING
                # The target is the velocity field: noise - clean_target_latents
                target = noise - target_latents
                
                # Compute base loss per pixel
                loss_per_pixel = (weighting.float() * (model_pred.float() - target.float()) ** 2)
                
                # Strategy 1: Core mask region - full reconstruction loss
                core_masked_loss = loss_per_pixel * mask_latent
                
                # Strategy 2: Boundary region - soft blending loss for harmonious transitions
                # Use soft mask for gradual transition from full loss to no loss
                boundary_masked_loss = loss_per_pixel * soft_mask
                
                # Combine losses with different weights
                # Core region: full weight for accurate reconstruction
                # Boundary region: reduced weight for smooth blending
                core_weight = 1.0
                boundary_weight = 0.5  # Reduced weight for boundary to allow some flexibility
                
                combined_loss = core_weight * core_masked_loss + boundary_weight * (boundary_masked_loss - core_masked_loss)
                
                # Compute mean loss over the effective mask region
                effective_mask = mask_latent + boundary_weight * (soft_mask - mask_latent)
                mask_sum = effective_mask.sum(dim=[1, 2, 3])  # Sum of effective mask per sample
                loss_sum = combined_loss.sum(dim=[1, 2, 3])  # Sum of loss per sample
                
                # Avoid division by zero - if no mask, use small epsilon
                mask_sum = torch.clamp(mask_sum, min=1e-8)
                loss_per_sample = loss_sum / mask_sum  # Average loss per effective masked pixel
