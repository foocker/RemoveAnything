...
def add_virtual_human_scores(batch):
    """
    为没有人工分数的batch添加虚拟分数（最大值）
    """
    batch_size = len(batch["ref"]) if isinstance(batch["ref"], list) else batch["ref"].shape[0]
    
    # 添加虚拟的人工分数（所有分数都设为最大值）
    batch["human_scores"] = {
        "removal_quality": [1.0] * batch_size,
        "background_quality": [1.0] * batch_size, 
        "smoothness": [1.0] * batch_size
    }
...
def compute_grpo_loss(model_pred, target, adapter_weights, rewards, old_log_probs, args):
    ...
    # 计算新策略的log概率
    new_log_probs = torch.log(adapter_weights + 1e-8).sum(dim=-1)
    ...
    # PPO风格的策略损失
    surr1 = ratio * total_reward
    surr2 = torch.clamp(ratio, 1.0 - args.clip_range, 1.0 + args.clip_range) * total_reward
    policy_loss = -torch.min(surr1, surr2).mean()
    
    # 多样性损失（鼓励专家权重分布的多样性）
    entropy = -(adapter_weights * torch.log(adapter_weights + 1e-8)).sum(dim=-1).mean()
    diversity_loss = -entropy  # 负熵作为多样性损失
    
    # 总损失
    total_loss = (args.base_loss_weight * base_loss + 
...
def compute_rewards(pred_images, target_images, masks, batch, args, device):
    """
    计算GRPO奖励（简化版本）
    ...
    # 简化的奖励计算（基于MSE）
    mse_loss = F.mse_loss(pred_images, target_images, reduction='none')
    mse_per_sample = mse_loss.view(batch_size, -1).mean(dim=1)
    
    # 将MSE转换为奖励（越小越好，所以取负数）
    removal_reward = -mse_per_sample
    background_reward = -mse_per_sample * 0.5  # 背景奖励权重较小
    smoothness_reward = -mse_per_sample * 0.3  # 平滑度奖励权重最小
    
    # 组合总奖励
    total_reward = (args.removal_reward_weight * removal_reward + 
                   args.background_reward_weight * background_reward + 
                   args.smoothness_reward_weight * smoothness_reward)
...
def parse_args():
    ...
    parser.add_argument("--clip_range", type=float, default=0.2,
                        help="PPO裁剪范围")
    parser.add_argument("--removal_reward_weight", type=float, default=1.0,
                        help="移除质量奖励权重")
    parser.add_argument("--background_reward_weight", type=float, default=0.5,
                        help="背景生成奖励权重")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if args.use_grpo and args.enable_grpo_loss:
                ...
                # 获取适配器权重
                if moe_manager is not None:
                    adapter_weights = moe_manager.current_weights
                    if adapter_weights is None:
                        # 如果没有权重，使用均匀分布
                        adapter_weights = torch.ones(imgs.shape[0], args.num_experts, device=accelerator.device) / args.num_experts
                    else:
                        # 将list转换为tensor
                        if isinstance(adapter_weights, list):
                            adapter_weights = torch.tensor(adapter_weights, device=accelerator.device)
                        # 扩展到batch维度
                        if adapter_weights.dim() == 1:
                            adapter_weights = adapter_weights.unsqueeze(0).expand(imgs.shape[0], -1)
                else:
                    # 没有MoE管理器时使用均匀分布
                    adapter_weights = torch.ones(imgs.shape[0], args.num_experts, device=accelerator.device) / args.num_experts
                ...
                # 计算旧策略的log概率（简化版本）
                old_log_probs = torch.log(adapter_weights + 1e-8).sum(dim=-1).detach()
                ...
                if accelerator.is_main_process and global_step % 100 == 0:
                    grpo_logs = {
                        "grpo/base_loss": loss_dict['base_loss'].item(),
                        "grpo/policy_loss": loss_dict['policy_loss'].item(),
                        "grpo/diversity_loss": loss_dict['diversity_loss'].item(),
                        "grpo/mean_reward": loss_dict['mean_reward'].item(),
                        "grpo/mean_ratio": loss_dict['mean_ratio'].item(),
                    }
                    accelerator.log(grpo_logs, step=global_step)
